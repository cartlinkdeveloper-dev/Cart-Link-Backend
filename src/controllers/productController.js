...
async function _sendFcm(tokens, payload) {
    ...
    try {
        ...
        console.error('[FirebaseAdmin] send error:', e);
    }
}
            ...
            try {
                ...
                if (shop && shop.followers && shop.followers.length > 0) {
                    ...
                    await _sendFcm(tokens, {
                        title: 'New Product Available',
                        body: `A new product "${product.name}" is available`,
                        data: { productId: product._id.toString(), shopId: product.ownerId.toString() }
                    });
                }
            } catch (notifErr) {
...
exports.updateProduct = async (req, res) => {
    try {
        ...
        const { name, description, price, mrp, stock, inStock, sku, category, isActive, isFeatured, images,
            color, size, material, weight, brand, length, width, height,
            // offer related
            offerPrice, offerStart, offerEnd, offerActive } = req.body;
        ...
        // Handle offer fields if provided
        if (offerPrice !== undefined) update.offerPrice = Number(offerPrice);
        if (offerActive !== undefined) update.offerActive = !!offerActive;
        if (offerStart !== undefined) {
            const ds = new Date(offerStart);
            if (!isNaN(ds.getTime())) update.offerStart = ds;
        }
        if (offerEnd !== undefined) {
            const de = new Date(offerEnd);
            if (!isNaN(de.getTime())) update.offerEnd = de;
        }

        const orig = await Product.findById(id).lean();
        const updated = await Product.findByIdAndUpdate(id, update, { new: true });
        if (!updated) return res.status(404).json({ success: false, message: 'Product not found' });

        // if offer was newly activated via update, notify followers
        if (updated.offerActive && !orig?.offerActive) {
            try {
                const shop = await Shop.findById(updated.ownerId).select('followers shopName').lean();
                if (shop && shop.followers && shop.followers.length > 0) {
                    const notifications = shop.followers.map(followerId => ({
                        customerId: followerId,
                        title: 'Special Offer on Product',
                        message: `An offer is now live on ${updated.name}!`,
                        type: 'offer',
                        data: { productId: updated._id, shopId: updated.ownerId }
                    }));
                    await Notification.insertMany(notifications);
                    ...
                    await _sendFcm(tokens, {
                        ...
                        body: `Check out this deal on ${updated.name}!`,
                        data: { productId: updated._id.toString(), shopId: updated.ownerId.toString() }
                    });
                }
            } catch (e) {
